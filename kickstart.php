...
// =====================================================================================================================
// XAMPP LOCAL RESTORE SETTINGS - configured for htdocs/cast1
// =====================================================================================================================
@ini_set('memory_limit', '256M');
@ini_set('max_execution_time', '120');
@set_time_limit(120);

// Force the extraction destination to this folder (cast1)
...
define('KICKSTART_MIN_PHP', '7.4.0');
...
if (@is_file($cacertpem) && @is_readable($cacertpem))
{
 ...
 */
function importKickstartFeatures($directory, $prefixes = ['kickstart'])
{
	...
	while ($filename = readdir($dh))
	{
		...
		{
			wincache_refresh_if_changed([$directory . '/' . $filename]);
		}
...
/**
 * Generates a random alphanumeric (or password) string.
 *
 * @param   bool  $forPassword  True to include special characters
 * @param   int   $length       Length of the string
 *
 * @return  string
 */
function ksGenRandomString(bool $forPassword = false, int $length = 16): string
{
	$salt   = $forPassword
	...
	for ($i = 1; $i <= $length; ++$i)
	{
		$return .= $salt[($shift + ord($random[$i])) % $base];
		$shift  += ord($random[$i]);
	}
...
/**
 * Loads and parses a JSON configuration file named {basename}.json.php or kickstart.json.php.
 * Populates the global $kickstartConfig array.
 *
 * @return  void
 */
function ksLoadConfigFile(): void
{
	global $kickstartConfig;

	$selfFile = defined('KSSELFNAME') ? KSSELFNAME : __FILE__;
	$parts    = explode('.', basename($selfFile));
	array_pop($parts);
	$selfBasename = implode('.', $parts);

	$possibleFiles = [
		dirname($selfFile) . DIRECTORY_SEPARATOR . $selfBasename . '.json.php',
		dirname($selfFile) . DIRECTORY_SEPARATOR . 'kickstart.json.php',
		KSROOTDIR . DIRECTORY_SEPARATOR . $selfBasename . '.json.php',
		KSROOTDIR . DIRECTORY_SEPARATOR . 'kickstart.json.php',
	];
	...
	$lines    = explode("\n", $fileData);
	$preamble = trim(array_shift($lines));
...
<p>
	Your Kickstart configuration file is missing the <code>&lt;?php die(); ?&gt;</code> line at the top of the file. Please address this issue and reload this page.
</p>
</body>
</html>
NOPE;
	...
	$kickstartConfig = $parsed;
}

ksLoadConfigFile();

// Apply debug mode from config file if not already defined
if (!defined('KSDEBUG') && !empty($kickstartConfig['debug']))
{
	define('KSDEBUG', 1);
...
$isCli = !(isset($_SERVER) && is_array($_SERVER)) || !array_key_exists('REQUEST_METHOD', $_SERVER);

// Make sure the file has been renamed, unless it is password-protected
$_ks_basename         = basename(__FILE__, '.php');
$_ks_nameHasKickstart = strpos($_ks_basename, 'kickstart') !== false;
$_ks_nameHasKs        = strpos($_ks_basename, 'ks') !== false;
$_ks_nameHasAkeeba    = strpos($_ks_basename, 'akeeba') !== false;

if (!$isCli && ($_ks_nameHasKickstart || $_ks_nameHasKs || $_ks_nameHasAkeeba))
{
	// If a password is configured, skip the rename check — the password protects access.
	global $kickstartConfig;

	$_ks_hasPassword = isset($kickstartConfig)
		&& is_array($kickstartConfig)
		&& isset($kickstartConfig['password'])
		&& !empty(trim($kickstartConfig['password']));

	if (!$_ks_hasPassword)
	{
		$_ks_random         = ksGenRandomString();
		$_ks_randomPassword = ksGenRandomString(true);
		$_ks_configFile     = $_ks_basename . '.json.php';
		$_ks_pwHash         = password_hash($_ks_randomPassword, PASSWORD_DEFAULT);
...
<body>
<h1>Akeeba Kickstart – Insecure setup detected</h1>
<p>
	Akeeba Kickstart has detected that its file name is <code>{$_ks_basename}.php</code> which can be easily guessed, and you are not using a password to protect access to it.
</p>
<p>
	You have two options to fix this problem.
</p>
<h4>Option 1. Use a password (recommended)</h4>
<p>
	Create a file named <code>{$_ks_configFile}</code> in the same directory as Kickstart. Put the following contents into that file:
</p>
<pre>
&lt;?php die(); ?&gt;
{
...
<p>
	Then, reload this page. You will be asked for a password. Enter <code>{$_ks_randomPassword}</code> and click the button to unlock Kickstart.
</p>
<h4>Option 2. Rename this file (legacy method; discouraged)</h4>
<p>
	Please change the file name to something which <em>does not</em> include any of the following terms:
</p>
<ul>
	<li><code>kickstart</code></li>
	<li><code>ks</code></li>
	<li><code>akeeba</code></li>
</ul>
<p>
	It is best to use a completely random filename. For example, you can rename this file to <code>{$_ks_random}.php</code>. Then you can access this file by replacing <code>{$_ks_basename}.php</code> with the new name in the address bar of your browser.
</p>
<p>
	If your site has a <code>.htaccess</code> or <code>web.config</code> file please remember to rename, or remove it to avoid it blocking access to your (renamed) Kickstart file. If you are using the NginX web server, please note that your server configuration may be blocking access to your (renamed) Kickstart file; in this case, you will need to edit your site's configuration.
</p>
<hr />
<h3>Why do you need to do that?</h3>
<p>
	Due to its nature, Akeeba Kickstart will execute commands sent to it by anybody accessing it. If Kickstart on your site has a well-known or easy-to-guess name, it is easy for an attacker to restart and hijack your restoration session. If you are using neither a password-protected JPS archive, nor a password-protected installer, the attacker can override your administrator / super user password with their own and take over your site. This is especially true if you forget to click on the Clean Up button at the end of the restoration, leaving Kickstart and your backup archive behind.
</p>
HTML;

		if (defined('KICKSTARTPRO') && KICKSTARTPRO)
		{
			echo <<< HTML
<p>
	Further to that, Akeeba Kickstart Professional allows you to import ZIP archives from arbitrary URLs and Amazon S3 buckets. Therefore, an attacker with access to it can use it to load malware to your site while you are restoring your site.
</p>
HTML;
		}
		...
		echo <<< HTML
<p>
	Using a strong password to protect access to Kickstart makes abusing Kickstart impractical for an attacker. Using a completely arbitrary filename for Kickstart makes it much harder for the attacker to guess which URL they need to visit to access Kickstart, mitigating this risk.
</p>
</body>
</html>
HTML;
...
unset(
	$_ks_basename, $_ks_nameHasKickstart, $_ks_nameHasKs, $_ks_nameHasAkeeba,
	$_ks_hasPassword, $_ks_random, $_ks_randomPassword, $_ks_configFile, $_ks_pwHash
);
...
class AKText extends AKAbstractObject
{
	...
	private $default_translation = [
		...
		'AKS3_ENDPOINT_CUSTOM'            => 'Use custom endpoint (for S3-compatible services)',
		'AKS3_ENDPOINT'                   => 'Endpoint',
		'AKS3_CONNECT'                    => 'Connect to Amazon S3',
		'AKS3_CANCEL'                     => 'Cancel import',
		'AKS3_TITLE_STEP2'                => 'Select your Amazon S3 bucket',
		'AKS3_BUCKET'                     => 'Bucket',
		'AKS3_SIGNATURE'                  => 'Signature method',
		'AKS3_SIGNATURE_V2'               => 'v2 (legacy)',
		'AKS3_SIGNATURE_V4'               => 'v4 (recommended)',
		'AKS3_REGION'                     => 'Region',
		'AKS3_REGION_CUSTOM'              => 'Custom',
		'AKS3_REGION_AUTODETECT'          => 'Auto-detect region (uncheck for S3-compatible services)',
...
class AKKickstartUtils
{
	...
	public static function findArchives($path)
	{
		...
		while (false !== $file = @readdir($dh))
		{
			...
			if (in_array($extension, ['jpa', 'zip', 'jps']))
			{
				$ret[] = $file;
			}
		}
...
function callExtraFeature($method = null, array $params = [])
{
	...
	foreach ($extraFeatureObjects as $o)
	{
		...
		$result = call_user_func([$o, $method], $params);
	}
...
function getListing($directory, $host, $port, $username, $password, $passive, $ssl)
{
	...
	$parsed_dir = trim($dir, '/');
	$parts      = empty($parsed_dir) ? [] : explode('/', $parsed_dir);
...
#gotoSite,
#gotoAdministrator,
#gotoPostRestorationTroubleshooting {
    margin: 0 2em 1.3em;
}
...
function onRealRunCleanupClick()
{
	...
	akeeba.System.doAjax(post, function (ret)
	{
		...
		document.getElementById("gotoPostRestorationTroubleshooting").style.display = "block";
	...
	<div id="page-container">
		...
		<div id="header">
			<div class="title">
				...
				Akeeba Kickstart <?php echo $edition ?> <?php echo defined('VERSION') ? VERSION : '9.1.0' ?>
			</div>
		</div>
		...
		<div id="page2">
			...
			<div id="extractionComplete" style="display: none">
				...
				<div id="gotoPostRestorationTroubleshooting" style="display:none">
...
function echoPasswordPage()
{
	$edition    = KICKSTARTPRO ? 'Professional' : 'Core';
	$selfFile   = defined('KSSELFNAME') ? KSSELFNAME : __FILE__;
	$parts      = explode('.', basename($selfFile));
	array_pop($parts);
	$configFile = implode('.', $parts) . '.json.php';
	?>
	<!DOCTYPE html>
	<html>
	<head>
		<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
		<title>Akeeba Kickstart <?php echo $edition ?> <?php echo defined('VERSION') ? VERSION : '9.1.0' ?></title>
		<style type="text/css" media="all" rel="stylesheet">
			<?php echoCSS();?>
		</style>
		<script>
			document.addEventListener('DOMContentLoaded', function() {
				document.getElementById('kspassword').focus();
			});
		</script>
	</head>
	<body>
				...
				<form method="post">
					<div class="step1">
						...
						<div class="area-container">
							...
							<p>
								Please enter your password to unlock this tool.
							</p>
							<p style="font-size: smaller">
								We remind you that your password is set in the following file:
								<code><?php echo htmlspecialchars($configFile) ?></code>
							</p>

							<label for="kspassword">
								Password
							</label>
							<span class="field">
								<input type="password" id="kspassword" name="kspassword"/>
							</span>
							<br/>
						</div>
						<div class="area-container">
							<label for="gobutton_top"></label>
							<button type="submit" class="button" style="padding: 0.5em 2em; margin: 0;">
								Unlock Kickstart
							</button>
						</div>
					</div>
...
function kickstart_application_cli()
{
	...
	if (!$silent)
	{
		$version = defined('VERSION') ? VERSION : '9.1.0';
		echo <<< BANNER
...
BANNER;
	...
	if (AKCliParams::hasOption('dump-language'))
	{
		$text        = AKText::getInstance();
		$langContent = $text->dumpLanguage();
		$langCode    = 'en-GB';
		$filePath    = KSROOTDIR . DIRECTORY_SEPARATOR . $langCode . '.kickstart.ini';
		$result      = file_put_contents($filePath, $langContent);

		if ($result === false)
		{
			if (!$silent)
			{
				echo "Failed to write language file to {$filePath}\n";
			}
			exit(255);
		}
		...
		if (!$silent)
		{
			echo "Language file written to {$filePath}\n";
		}
		exit(0);
	}

	$paths = AKCliParams::getOption('', [], true);
...
BANNER;
	...
	AKFactory::set('kickstart.setup.renamefiles', []);
...
/**
 * Returns the configured Kickstart password, or null if none is set.
 */
function ksGetPassword(): ?string
{
	global $kickstartConfig;

	return trim($kickstartConfig['password'] ?? '') ?: null;
}

/**
 * Returns true if the current visitor has authenticated with the Kickstart password.
 */
function ksIsLoggedIn(): bool
{
	return $_SESSION['kickstart.logged_in'] ?? false;
}

/**
 * Returns true if the password page must be shown before granting access.
 */
function ksMustAskForPassword(): bool
{
	return ksGetPassword() && !ksIsLoggedIn();
}

/**
 * Attempts to log in with the given password.
 */
function ksLogin(?string $password): void
{
	if (empty($password))
	{
		return;
	...
	$storedPassword = ksGetPassword();

	// Refuse to accept the documentation example password.
	if (
		hash_equals('2EnNP6QXKNCUfERfvt5am5rUNzy3gaPk', $storedPassword)
		|| @password_verify('2EnNP6QXKNCUfERfvt5am5rUNzy3gaPk', $storedPassword)
	)
	{
		echo <<< NOPE
<html lang="en">
...
<h1>Invalid Configuration</h1>
<p>
	You tried using the example password <code>2EnNP6QXKNCUfERfvt5am5rUNzy3gaPk</code> found in the documentation. We explicitly told you not to do that.
</p>
</body>
</html>
NOPE;
	...
	try
	{
		$isLoggedIn = @password_verify($password, $storedPassword);
	}
	catch (Throwable $e)
	{
		$isLoggedIn = false;
	...
	$_SESSION['kickstart.logged_in'] = $isLoggedIn || hash_equals($storedPassword, $password);
}

/**
 * Logs out by clearing the session flag.
 */
function ksLogout(): void
{
	$_SESSION['kickstart.logged_in'] = false;
}
...
function kickstart_application_web()
{
	...
	switch ($task)
	{
		...
		case 'display':
			$ajax       = false;
			$ksPassword = getQueryParam('kspassword');

			if (!empty($ksPassword) && ksGetPassword())
			{
				ksLogin($ksPassword);
			}

			if (ksMustAskForPassword())
			{
				echoPasswordPage();
			}
			else
		...
		case 'dumpLang':
			...
			$langCode = 'en-GB';
			$filePath = KSROOTDIR . DIRECTORY_SEPARATOR . $langCode . '.kickstart.ini';
			$writeResult = file_put_contents($filePath, $langContent);
			$retArray = [
				'filePath' => $filePath,
